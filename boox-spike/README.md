...
- **文字圖層** — 對話。你寫給 oracle 的話住在這層，停筆 2.8s 送出的
  **同時整層開始溶解**（快照已經把話帶走了，不等回覆）；oracle 的文字
  回覆也寫在這層，讀完後自己溶解。
...
停筆後整頁（兩層合成）截圖送給 oracle。座標是誠實的：模型收到快照的
真實像素尺寸，回覆裡每個 `x y` 都是快照裡的像素位置（它看到的框就是它
畫的框）。回覆是純文字 DSL：`TEXT x y … END_TEXT` 是說話（寫進文字圖層，
會溶解）、`STROKE / P x y / END_STROKE` 是畫畫——prompt 只教格式、不設
畫圖規則（它是自由發揮的畫家）；落筆時以 centripetal Catmull-Rom 曲線
平滑通過它給的每個點再重取樣，筆徑跟手寫同粗（藍色墨水寫進畫筆圖層，
留在紙上），兩種 block 邊串流邊逐筆寫出。文法與解析見 `ReplyDsl.kt`，
prompt 在 `Oracle.kt` 的 `SYSTEM_PROMPT`。

對話是有記憶的：每輪的問答以純文字跟著下一次請求（只有當回合帶圖，
payload 不隨回合膨脹）。因為頁面上的字會消失，每則回覆都以一個不落墨的
`SEE` block 開頭——首輪完整描述整頁、之後每輪抄下新增的黑墨——這就是
使用者那半邊對話的永久記錄。按 **Clear** 才會結束 session：清掉兩層墨水
和 oracle 的記憶。
