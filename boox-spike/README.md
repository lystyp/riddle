...
- **文字圖層** — 對話。你寫給 oracle 的話住在這層，停筆 2.8s 送出的
  **同一瞬間清空**（快照已經把話帶走了）；oracle 的文字回覆也寫在這層，
  讀完後自己溶解。
...
會溶解）、`STROKE / P x y / END_STROKE` 是畫畫——模型只給 3~12 個錨點，
落筆時以 centripetal Catmull-Rom 曲線平滑通過所有錨點再重取樣，筆徑跟
手寫同粗（藍色墨水寫進畫筆圖層，留在紙上），兩種 block 邊串流邊逐筆
寫出。文法與解析見 `ReplyDsl.kt`，prompt 在 `Oracle.kt` 的 `SYSTEM_PROMPT`。
